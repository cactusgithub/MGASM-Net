...
def tree_parse():
    # input_path = "./Data/BAS/val/image1/" #ground truth or predictions
    input_path = "/dev/raid/doctor/xy/DataSets/Parse22/val_image/"
    # save_path = "./Data/BAS/val/ske/"
    save_path = "/dev/raid/doctor/xy/DataSets/Parse22/val_ske/"
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    file_list = os.listdir(input_path)
    # input_path1 = './Data/BAS/val/label1/';
    input_path1 = "/dev/raid/doctor/xy/DataSets/Parse22/val_label/"
    file_list.extend(os.listdir(input_path1))
    file_list.sort()
    ...
    #ids = 10
        ...
        # img = img.get_fdata()
        # label = nibabel.load(input_path1+file_list[2*ids+1])
        # label = label.get_fdata()

        label_itk = sitk.ReadImage(input_path1+file_list[2*ids+1])
        label = sitk.GetArrayFromImage(label_itk)
        label = (label>0).astype(np.uint8)
        
        label = large_connected_domain(label)
        skeleton = skeletonize_3d(label).astype(int)
        ...
        save_name_parse = save_path + file_list[2*ids][:8] + "_parse.nii.gz"
        save_name_skel = save_path + file_list[2*ids][:8] + "_skel.nii.gz"
        ...
        parse_nii.CopyInformation(label_itk)
        sitk.WriteImage(parse_nii,save_name_parse)
        skel_nii = sitk.GetImageFromArray(skeleton)
        skel_nii.CopyInformation(label_itk)
        sitk.WriteImage(skel_nii,save_name_skel)
        ...
        save_name_pm = save_path + file_list[2*ids][:8] + "_parent.npy"
        save_name_cm = save_path + file_list[2*ids][:8] + "_children.npy"
        np.save(save_name_pm, parent_map)
        np.save(save_name_cm, children_map)

if __name__=='__main__':
    # for i in range(10, 130, 10):
    #     # path = "./ unresampling / result(aunet) /0" + str(i) + ".ckpt"
    #     resume = "./unresampling/result(aunet)/010.ckpt"
    #     path = resume.split("0")[0] + "0" + str(i) + ".ckpt"
